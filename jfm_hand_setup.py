...
#Creates a window that automates a number of tasks for rigging a character hand
...
def transformObjects(objectToTransform, transformValues):
	...
	cmds.move(translateValues[0], translateValues[1], translateValues[2], objectToTransform)
...
def duplicateJoints():
	...
	while(jointDupeNumTimes > 0):
		newObject = cmds.duplicate(selectionList, n='temp', rc=True) #the objects have to be renamed because there is nothing you can do with them until they have a different name from their originals
		newObject = newObject[0]
		newObjectList.append(newObject) #list of the roots of each duplicated hand

		#The folloiwing lines select any shape nodes in the duplicate joint hierarchy and deletes them
		shapeSelection = cmds.listRelatives(newObject, allDescendents = True, type = 'shape' ) #creating a list to select any unwanted shapes in the hierarchy
	...
	newObjectList.append(selectionList)
	prefixNumber = 0
	duplicatedHandList = []
...
def renameHand(prefixNumber, selectedJoint, duplicatedHandList):
	'''
	This function is itterated through to rename each duplicate hand's joints appropriately
	'''
	rootPrefixList = {5 : 'SkinRoot', 4 : 'DrivenRoot', 3 : 'ControlRoot', 2 : 'IKRoot', 1 : 'RigRoot' } 
	fingerPrefixList = {5 : 'Skin', 4 : 'Driven', 3 : 'Control', 2 : 'IK', 1 : 'Rig' } 

	#This composes the name for the selected joint chain
	prefixName = selectPrefix(prefixNumber, rootPrefixList)#this takes the prefix number and the prefixList dictionary and compares until it returns the appropriate prefix
	rootJoint = cmds.rename(selectedJoint, prefixName + '_' + globalSideOfBody + '_' + 'HandRoot' )#this does the actual renaming of the root joints but also stores the joint after renaming it.
	...
	fingerRootList = cmds.listRelatives(rootJoint, children = True) # this creates a list of the fingers under the hand's root
	...
	for eachJoint in fingerRootList:
		...
		fingerPrefixName = selectPrefix(prefixNumber, fingerPrefixList)#this calls a function that returns an appropriate value for the prefix of the joint chain
...
def parentControlJoints_DrivenJoints(drivenRootJoint, controlRootJoint):
	drivenJointFingerList = cmds.listRelatives(drivenRootJoint, children=True)
	controlJointFingerList = cmds.listRelatives(controlRootJoint, children=True)
...
def parentConstrainHierarchy(targetJointsList, controlledJoints):
	...
	if(len(targetJointsList) > 1):
		...
		controlledJointChildren = cmds.listRelatives(controlledJoints, children=True)
		...
		controlledJointChildrenList = cmds.listRelatives(controlledJoints, children=True)
...
def createIkHandles(jointRoot):	
	pathToFile = '%s/%s' % (scriptsUserDirectory.rsplit('/', 3)[0], 'assets/jfm_transformControl.mb')#regroving that path to go to the assets file
	loadedControlShape = cmds.file(pathToFile, i = True)
	fingerRootList = cmds.listRelatives(jointRoot, children = True)
	ikControlGroup = cmds.group(empty = True, name = globalSideOfBody + '_IKControlGroup')
	snapToObj(jointRoot, ikControlGroup)
	fingerRootIndex = 0
	for eachJoint in fingerRootList:
		objectType = cmds.objectType(eachJoint)
		fingerJointList = cmds.listRelatives(eachJoint, allDescendents = True)
		...
		newControlShape = cmds.duplicate('jfm_transformControl', name = eachJoint + '_IkHandle')
		...
		cmds.parent(newControlShape, ikControlGroup)
...
pathToFile = '%s/%s' % (scriptsUserDirectory.rsplit('/', 3)[0], 'assets/jfm_handControlbox.mb')#regroving that path to go to the assets file
loadedControlShape = cmds.file(pathToFile, i = True)
...
loadedControlShape = cmds.rename('jfm_handControlBox', globalSideOfBody + '_' + 'Hand_Control_Box')
...
globalDuplicatedHandListIndex = 0
for eachjoint in globalDuplicatedHandList:
	cmds.parent(globalDuplicatedHandList[globalDuplicatedHandListIndex], handControllsNull)
	globalDuplicatedHandListIndex = globalDuplicatedHandListIndex + 1
cmds.parentConstraint(skinRootJoint, handControllsNull)
