...
def loadSelected(loadSelected):
	...
	newSelection = cmds.textField('selectedObjectField', edit = True, text = rigToDupe[0].encode('utf8'))
	print('loadSelected, the globalRigToDupe is: ' + str(rigToDupe)) 
	global globalRigToDupe
	globalRigToDupe = rigToDupe
...
def dupeRig(dupeRig, pNameSearchField, pNameReplaceField):
	...
	if (objectType != 'joint'):
		...
		reps = {searchForField: replaceWithField, 'SknNoWeight':'Rig', '1': ''}
		...
		reps = {searchForField: replaceWithField, 'SknNoWeight': 'Rig', 'SknGeo' : 'Rig', 'Hair':'Rig_Hair'}
...
nameReplaceField = cmds.textField(text='Rig')
